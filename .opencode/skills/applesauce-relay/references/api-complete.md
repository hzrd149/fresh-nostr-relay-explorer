...
Create a relay pool with optional default relay options that apply to all
relays.
...
Created via `pool.group()`. Provides same interface as Relay but for multiple
relays.
...
function completeOnEose(
  includeEose: true,
): MonoTypeOperatorFunction<SubscriptionResponse>;
function completeOnEose(
  includeEose: false,
): OperatorFunction<SubscriptionResponse, NostrEvent>;
```
