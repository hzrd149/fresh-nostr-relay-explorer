...
async function publishAndTrack(event: NostrEvent) {
  ...
  if (failed.length > 0) {
    console.warn("Failed relays:", failed.map(r => ({
      relay: r.from,
      reason: r.message
    })));
  }
...
async function syncWithMultipleRelays(
  ...
  pool.sync(relays, store, filters, "down")
    ...
    .subscribe({
      complete: () => {
        console.log(`Total synced: ${totalSynced} events from ${relays.length} relays`);
      }
    });
}
```
...
export default function AuthenticatedIsland({ relay }: { relay: string }) {
  const authStatus = useSignal<"pending" | "authenticated" | "failed">("pending");
...
filters$.pipe(
  switchMap(filters => 
    relay.subscription(filters, { id: "dynamic-feed" })
  )
).subscribe(response => {
  if (response !== "EOSE") {
...
```typescript
class EventCache {
  private cache = new Map<string, { events: NostrEvent[], timestamp: number }>();
